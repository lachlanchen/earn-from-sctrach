...
## 2. Three-Layer Playbook

1. **Lazy Money Layer** – High-autonomy assets such as:
   - Subscription research drops (`$49/mo`) delivered via PDF + on-site viewer.
   - “White-glove automation vaults” (Zapier, Notion, Airtable bundles) licensed to boutique agencies.

2. **Lazy Earn Layer** – Systems that keep leads and delivery on autopilot:
   - **Idea Playground v2:** embed an API key to store generated ideas, email the visitor the result, and tag them in the CRM.
   - **Research Viewer analytics:** log each PDF view to determine which tickers or ideas deserve deeper follow-ups.

3. **Earn From Scratch Layer** – Low-stakes weekly experiments:
   - `Friday Drift`: 60-minute livestream where you narrate a lazy money build in public.
   - `Spark → Draft → Deck`: capture a single line (Spark), expand to 200-word Drift, then repackage into a minimalist Keynote deck for LinkedIn Slides.
...
2. **Q2 – Dossier Double-Release**
   ...
   - Pair each release with a limited-run concierge offer to capture premium demand.

3. **Q3 – Freedom Scenarios**
   - Model three scenarios: Chill ($10K/mo), Float ($25K/mo), Drift ($50K/mo). Map which products/loops cover each scenario.
   - Adjust pricing or packaging so that the “Chill” scenario is evergreen even if all other projects pause.

4. **Q4 – Detox Sprint**
   - Stop launching, only refine existing products and research.
   - Use the time to migrate any critical systems from third-party tools to self-hosted or multi-provider setups.
...
- [ ] **Set up a dedicated analytics dashboard** (Plausible/GoatCounter) for the new `research-viewer.html` page to capture PDF engagement.
- [ ] **Add `financial_freedom.md` to the site** via an excerpt or embedded card in the Research section.
- [ ] **Record a walkthrough video** (loom) summarizing this blueprint and embed it near the top to entice casual visitors.
